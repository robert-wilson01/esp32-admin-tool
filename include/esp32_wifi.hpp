...
//Creacion de un hostname para ESPmDNS.Usar http://adminesp32.local
const char *esp_hostname = id;

void startAP(){
    log("Informacion: Iniciando Modo AP");
    WiFi.mode(WIFI_STA);
    WiFi.disconnect();
    delay(100);    
...
void wifi_setup(){
    ...
    // 2) Caso contrario en Modo Cliente, o STA o tambien llamado modo estación
    else {
        WiFi.mode(WIFI_STA);
        wifi_mode = WIFI_STA;
        startClient();
    ...
    if (wifi_mode == WIFI_STA || wifi_mode == WIFI_AP){
        if (MDNS.begin(esp_hostname)) {
            MDNS.addService("http", "tcp", 80); //Sea grega servicio http, por el tcp en el puerto 80
        }
    } 
}
// -------------------------------------------------------------------
